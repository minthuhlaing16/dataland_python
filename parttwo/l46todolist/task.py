...
class TaskJob:
    ...
    def taskreport(self):
        ...
        print(
            f"Total tasks: {total}, Completed: {len(completed)} Percent: {percent:.2f}%"
        )
        highpriorities = reduce(
            lambda x, y: x + (1 if y.priority >= 5 else 0), self.tasks, 0
        )
    ...
    def savetasks(self, filename):
        try:
            ...
            print(f"Error: Type Error: {str(e)}")
